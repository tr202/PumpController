...
uint8_t iniStatus =0;
uint8_t hi2cReadyFlag = 1;
uint8_t i2cReadyFlag=1;
//------------------------------------------------
...
void delayTicks(uint32_t tick){
uint32_t tickHal = HAL_GetTick();
while (abs(HAL_GetTick() - tickHal)< tick);
...
bool i2cReady(){
	...
	if(HAL_I2C_GetState(&hi2c1) != HAL_I2C_STATE_READY) {return false;}else{return true;}
}
...
bool LCD_WriteByteI2CLCD(uint8_t bt)
{
	...
	if(HAL_I2C_Master_Transmit_DMA(&hi2c1,(uint16_t) 0x40,buf,1)!=HAL_OK) {
		iniStatus=0;
		return false;
	}
